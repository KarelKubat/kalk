...
#ifdef __unix__
    ch = '\0';
    while ((c = (int) read(0, &ch, (size_t) 1)) == -1) {
	if (errno != EINTR)
	    break;
    }
    if (c != (-1))
...
#endif
	tv.tv_sec = (time_t) (tlen / 10);
	tv.tv_usec = (suseconds_t) ((tlen % 10) * 100000);
...
gl_putc(int c)
{
    ...
    (void) write(1, &ch, (size_t) 1);
    if (ch == '\n') {
	ch = '\r';
        (void) write(1, &ch, (size_t) 1); /* RAW mode needs '\r', does not hurt */
    }
}
